...
public class Settings extends YamlSerializable {
  ...
  public static class TOOLS {
    ...
    public HUB_SPREAD HUB_SPREAD = new HUB_SPREAD();
    ...
    @Comment({"Hub Splitter configuration section."})
    public static class HUB_SPREAD
    {
      public boolean ENABLED = true;
      public List<String> SERVERS = List.of("hub-1", "hub-2"); @Comment({"Possible methods: 'LOWEST', 'CYCLE'"})
      public String HUB_SPREAD_METHOD = "LOWEST"; } } public static class PROTOCOL_BLOCKER { public boolean BLOCK_JOIN = true; public boolean BLOCK_PING = false; @Comment({"If true, all protocols except those listed below will be blocked."}) public boolean WHITELIST = false; @Comment({"You can set either a protocol number here (e.g. '340' for 1.12.2) or a Minecraft version below (e.g. '1.12.2')", "You can find a list of protocols here: https://wiki.vg/Protocol_version_numbers"}) public List<Integer> PROTOCOLS = List.of(Integer.valueOf(9999), Integer.valueOf(9998)); @Comment({"List of versions:", "1.7.2, 1.7.6, 1.8, 1.9, 1.9.1, 1.9.2, 1.9.4, 1.10, 1.11, 1.11.1, 1.12, 1.12.1, 1.12.2,", "1.13, 1.13.1, 1.13.2, 1.14, 1.14.1, 1.14.2, 1.14.3, 1.14.4, 1.15, 1.15.1, 1.15.2,", "1.16, 1.16.1, 1.16.2, 1.16.3, 1.16.4, 1.17, 1.17.1, 1.18, 1.18.2, 1.19, 1.19.1, 1.19.3, LATEST"}) public List<String> VERSIONS = List.of("1.7.2"); public String MINIMUM_VERSION = "1.7.2"; public String MAXIMUM_VERSION = "LATEST"; @Comment({"For \"block-ping\" option."}) public String BRAND = "Version is not supported!"; @Comment({"For \"block-ping\", set to \"\" to disable."}) public String MOTD = "&cVersion is not supported!{NL}&ePlease, join with Minecraft 1.12.2 or newer."; @Comment({"For \"block-joining\" option."}) public String KICK_REASON = "&cYour version is unsupported!"; } public static class BRAND_CHANGER { public boolean REWRITE_IN_PING = true; public boolean REWRITE_IN_GAME = true; public String PING_BRAND = "YourServer 1.12.2-1.19.3"; @Comment({"For ping."}) public boolean SHOW_ALWAYS = false; @Comment({"{0} - Original server brand (e.g. Paper)."}) public String IN_GAME_BRAND = "YourServer ({0})"; } @Comment({"Doesn't work with srv records."}) public static class HOSTNAMES_MANAGER { public boolean BLOCK_JOIN = true; public boolean BLOCK_PING = true; @Comment({"Connections IP logging."}) public boolean DEBUG = false; @Comment({"For \"debug\" option."}) public boolean SHOW_BLOCKED_ONLY = false; @Comment({"For \"block-joining\" option, set to \"\" to show the default reason."}) public String KICK_REASON = "&cPlease, don't connect to the direct ip!{NL}Use example.com"; public boolean WHITELIST = true; @Comment({"IP Addresses starting with \"127.\" or equal to \"localhost\" will be blocked."}) public boolean BLOCK_LOCAL_ADDRESSES = true; @Comment({"DoMaIn.net will be similar to domain.net."}) public boolean IGNORE_CASE = true; public List<String> HOSTNAMES = List.of("your-domain.net", "your-domain.com"); @Comment({"List of IP addresses that will bypass this check."}) public List<String> IGNORED_IPS = List.of("79.555.*", "228.1337.*"); } @Comment({"Hub Splitter configuration section"}) public static class HUB_SPREAD { @Comment({"Possible methods: 'LOWEST', 'CYCLE'"}) public String HUB_SPREAD_METHOD = "LOWEST";
    public boolean ENABLED = true;
    public List<String> SERVERS = List.of("hub-1", "hub-2"); }
